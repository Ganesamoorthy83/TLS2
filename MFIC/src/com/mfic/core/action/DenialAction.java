...
public class DenialAction extends ActionSupport  {
	...
	public void docUpload(){
		try {
		dataDir= dataDir + "LID-" + application.getLid() + "-" + "LOAN_DENIAL_LETTER" + "-" + docFileName;
	...
	/**
	 * To change status to loan denied .
	 * @return String
	 */
	@SuppressWarnings({ "rawtypes" })
	public String changeStatusToLoanDenied(){
		log.debug("change status to loan denied  action");
		String str;
		try{
			...
			if(doc != null){
				...
				addActionError(getText("denialLetter.required"));
			...
			if (application != null){
				if(application.getStep() == LoanApplicationStatus.APPLICATION_COMPLETE.getCode()){
					...
					application.setDenialNote(denialNote);
					application.setDenialDoclink(dataDir);
					application.setStep(LoanApplicationStatus.LOAN_DENIED.getCode());
					...
					if(emailList != null){
						...
						notificationToBorrower.sendMailToBorrowerDenial(emailList);
					}
			...
			addActionError(getText("denial.fail"));
			log.error("change status to loan denied  action failed", re);
			str = "fail";
		}
		return str;
	}
